...
def update_plot(frame, points, marker_indices, skeletal_connections, scatter_points, lines, bat_points, ax, num_frames, marker5_trace, trace_line):
    x = points[0, :, frame]
    y = points[1, :, frame]
    z = points[2, :, frame]
    ...
    # Update Marker5 trace
    marker5_idx = marker_indices["Marker5"]
    marker5_trace[0].append(x[marker5_idx])
    marker5_trace[1].append(y[marker5_idx])
    marker5_trace[2].append(z[marker5_idx])
    trace_line.set_data(marker5_trace[0], marker5_trace[1])
    trace_line.set_3d_properties(marker5_trace[2])

    if frame == num_frames - 51:
        marker5_trace[0].clear()
        marker5_trace[1].clear()
        marker5_trace[2].clear()

    ax.set_title(f"Frame {frame} of {num_frames}")
    return scatter_points, *lines, bat_points, trace_line
...
def main(user_id, session_id, height, weight, side, swing_number, exit_velo, repo_root_path, approx_swing_init_frame=400):
    ...
    marker5_trace = ([], [], [])  # Initialize trace storage for marker5
    trace_line, = ax.plot([], [], [], color='red', alpha=0.5)

    anim = FuncAnimation(fig, update_plot, frames=range(approx_swing_init_frame, num_frames-50), 
                         fargs=(points, marker_indices, skeletal_connections, scatter_points, lines, bat_points, ax, num_frames, marker5_trace, trace_line), 
                         interval=10, repeat=True)
    # anim.save('test_swing_animation.mp4', writer='ffmpeg')
    plt.show()

if __name__ == "__main__":
    file_path = "/Users/leofeingold/Documents/GitHub/openbiomechanics/baseball_hitting/data/c3d/000145/000145_000042_76_213_R_006_1039.c3d"
    #"/Users/leofeingold/Documents/GitHub/openbiomechanics/baseball_hitting/data/c3d/000059/000059_000017_67_179_R_006_974.c3d" 
